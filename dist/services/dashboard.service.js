...
async function getDashboardData(userId) {
    ...
    let managementScore = "needs_improvement";
    if (savingsRate >= 0.2)
        managementScore = "excellent";
...
async function generateRecommendations(userId) {
    ...
    const incomeAgg30 = await database_1.prisma.transaction.aggregate({
        _sum: { amount: true },
        where: {
            userId,
    ...
    const recent7dExpenses = await database_1.prisma.transaction.findMany({
        where: {
            userId,
    ...
    const recommendations = topCategories
        ...
        const absoluteMin = 1000000; // Rp 1.000.000 minimum akal sehat
        ...
        if (isEssentialFood && c.amount < baselineAmount) {
            ...
            return {
                ...
                message: `Pengeluaran saat ini Rp ${c.amount.toLocaleString("id-ID")} terlihat rendah untuk kebutuhan esensial 30 hari. Pertimbangkan menaikkan ke sekitar Rp ${suggested.toLocaleString("id-ID")} (~${(baselinePct * 100).toFixed(0)}% dari income).`,
                priority,
                category: c.name,
                amount: c.amount,
            };
    ...
    const foodLike = (n) => (n || "").toLowerCase().match(/food|dining|grocer|makan|sembako|kebutuhan pokok/);
    ...
    const expenseAgg30 = await database_1.prisma.transaction.aggregate({
        _sum: { amount: true },
    ...
    const goals = await database_1.prisma.goal.findMany({ where: { userId, isActive: true } });
